...
class SowprogEventsOutput {
	function output_main_page() {
		...
		<form action="<?php echo $sowprogEventsConfiguration->getAgendaPageFullURL(); ?>" method="get">
			...
			<input type="text" id="swc_query" name="swc_query" value="<?php echo isset($query['swc_query']) ? $query['swc_query'] : NULL; ?>" placeholder="Recherche">
			<button type="submit">
		...
		$swc_event = isset($query['swc_event']) ? $query['swc_event'] : NULL;
		$swc_location = isset($query['swc_location']) ? $query['swc_location'] : NULL;
		$swc_search_by_event_type = isset($query['swc_search_by_event_type']) ? $query['swc_search_by_event_type'] : NULL;
		$swc_search_by_event_style = isset($query['swc_search_by_event_style']) ? $query['swc_search_by_event_style'] : NULL;
		$swc_start_page = isset($query['swc_start_page']) ? $query['swc_start_page'] : NULL;
		$swc_date = isset($query['swc_date']) ? $query['swc_date'] : NULL;
		$swc_query = isset($query['swc_query']) ? $query['swc_query'] : NULL;
		$count = '20';
